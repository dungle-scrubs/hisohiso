...
final class AudioKitRecorder: @unchecked Sendable, AudioRecording {
    ...
    func startRecording() throws {
        ...
        guard let engine, let input = engine.input else {
    ...
    func stopRecording() -> [Float] {
        ...
        let resampledSamples: [Float] = if abs(Settings.sampleRate - targetSampleRate) > 1 {
            resample(samples, from: Settings.sampleRate, to: targetSampleRate)
        } else {
            samples
        }
        ...
        logInfo(
            "AudioKit recording stopped, captured \(samples.count) samples (\(Double(normalizedSamples.count) / targetSampleRate) seconds)"
        )
