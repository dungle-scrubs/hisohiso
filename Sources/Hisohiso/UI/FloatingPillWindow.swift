...
/// Animated waveform view using Core Animation.
private class WaveformView: NSView {
    private var barLayers: [CALayer] = []
    private var barsCreated = false
    // bar_width=3, bar_gap=2, max_height=20, min_height=4
...
///
/// Shows recording state and a live waveform while dictating. Visibility is
/// controlled by the "Show floating pill" preference (errors always show).
final class FloatingPillWindow: NSWindow {
    ...
    func show(
        ...
        onRetry: @escaping @Sendable @MainActor () -> Void
    ) {
