...
///
/// ## Thread safety
/// `stateStore` owns lifecycle state and captured samples behind one lock.
/// Audio tap callbacks run on the audio render thread; public API is called from
/// `@MainActor`. The state-store lock is held only for short reads/writes —
/// never across I/O or engine operations.
final class AudioRecorder: @unchecked Sendable, AudioRecording {
    private let engine = AVAudioEngine()
    private let stateStore = AudioRecorderStateStore()

    /// Thread-safe read for current state.
    private var state: AudioRecorderStateStore.Lifecycle {
        get { stateStore.state() }
        set { stateStore.setState(newValue) }
    }
    ...
    static func availableInputDevices() -> [AudioInputDevice] {
        ...
        for deviceID in deviceIDs {
            ...
            guard status == noErr, let name = nameRef?.takeUnretainedValue() else { continue }
    ...
    func stopRecording() -> [Float] {
        ...
        let samples = stateStore.drainSamples()
    ...
    func getRecentSamples(count: Int) -> [Float] {
        stateStore.recentSamples(count: count)
    }
    ...
    private func processAudioBuffer(_ buffer: AVAudioPCMBuffer, inputSampleRate: Double) {
        ...
        stateStore.appendSamples(resampledSamples)
    }
