...
final class EventTapManager: @unchecked Sendable {
    ...
    private let registry = EventTapRegistry()
    ...
    func register(
        ...
        registry.register(id: id, eventTypes: eventTypes, handler: handler)

        // Only restart the tap if the combined event mask changed
        if eventTap != nil {
            let newMask = registry.eventMask()
    ...
    private func startTap() -> Bool {
        let eventMask = registry.eventMask()
        ...
        logInfo("EventTapManager: started with event mask: \(eventMask)")
        return true
    }
    ...
    private func dispatch(event: CGEvent, type: CGEventType) -> Bool {
        registry.dispatch(event: event, type: type)
    }
}
