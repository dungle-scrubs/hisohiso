...
final class OpenAIProvider: CloudProvider {
    let id = "openai"
    ...
    var isConfigured: Bool {
        KeychainManager.shared.hasAPIKey(.openAI)
    }

    func transcribe(_ audioSamples: [Float]) async throws -> String {
        guard let apiKey = KeychainManager.shared.getAPIKey(.openAI) else {
        ...
        logInfo("OpenAI: sending \(wavData.count) bytes of audio")
        ...
        do {
            ...
            case 200:
                ...
                logInfo("OpenAI: transcription complete")
                return text
            ...
            default:
                ...
                logError("OpenAI: API error \(httpResponse.statusCode): \(errorMessage)")
